// SentinelGuard background script (Firefox / MV3).
//
// Firefox implements the `browser.*` namespace natively with Promises,
// so no polyfill is needed here (unlike Chrome/Edge's `chrome.*`).
//
...
async function setBadgeConnected(connected) {
  ...
  if (!connected) {
    ...
    await browser.action.setTitle({ title: "SentinelGuard: agent unreachable" });
  } else {
    await browser.action.setTitle({ title: "SentinelGuard" });
  }
}
...
browser.webNavigation.onBeforeNavigate.addListener(async (details) => {
  ...
  if (url.startsWith(browser.runtime.getURL(""))) return; // ignore our own pages
